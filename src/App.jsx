...
function FiadoRow({ f, onCobrar, onDelete, dimmed, th }) {
  return (
    <div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 12px",marginBottom:8,background:"rgba(255,255,255,0.03)",borderRadius:11,border:`1px solid rgba(255,255,255,0.07)`,opacity:dimmed?0.5:1}}>
      <div style={{flex:1}}>
        <div style={{fontSize:13,fontWeight:600,color:th?.text||"#e2e8f0",textDecoration:dimmed?"line-through":"none"}}>{f.nombre}</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
          {[["u20","20L",p20,"canje20"],["u12","12L",p12,"canje12"]].map(([field,label,price,cf])=>(
            <GCard key={field} th={th} style={{marginBottom:0,padding:"10px 12px"}}>
              ...
              {venta[field]>0&&venta.pago!=="fiado"&&(
...
export default function App() {
  ...
  const confirmarEntrega=async(pedido,{pago,canje20,canje12,nota})=>{
    ...
    if(pago==="fiado"){
      ...
      const nf=[...fiados,{id:Date.now(),nombre:pedido.nombre,clienteId:pedido.clienteId,monto,nota,fecha:todayKey(),cobrado:false,ventaId:venta.id}];
  ...
  const addVenta=async(venta)=>{
    ...
    if(venta.pago==="fiado"){
      ...
      const nf=[...fiados,{id:Date.now(),nombre:venta.nombre,clienteId:venta.clienteId,monto,nota:venta.nota,fecha:selectedDate,cobrado:false,ventaId:venta.id}];
      setFiados(nf); await sset("fiados_v2",nf);
    }
    const deudas=[];
    if(!venta.canje20&&(venta.u20||0)>0&&venta.pago!=="fiado") deudas.push({tipo:"20L",cant:venta.u20});
    if(!venta.canje12&&(venta.u12||0)>0&&venta.pago!=="fiado") deudas.push({tipo:"12L",cant:venta.u12});
  ...
  const cobrarFiado=async(fiado,fecha)=>{
    ...
    setFiados(nf); await sset("fiados_v2",nf);
    const targetDay=history.find(d=>d.date===fecha)||emptyDay(fecha);
